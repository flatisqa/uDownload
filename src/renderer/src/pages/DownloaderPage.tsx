    ...
    <div className="page">
            ...
            <div className="glass-panel" style={{ padding: 20 }}>
              ...
              <div className="flex gap-8 items-center" style={{ marginBottom: 12 }}>
                <button
                  className="btn btn-ghost"
                  style={{ padding: '4px 8px', fontSize: 11 }}
                  onClick={() => setSelectedChapters(meta.chapters!.map((c) => `${c.startTime}-${c.endTime}`))}
                >
                  {t('selectAll')}
                </button>
                <button
      ...
      {step === 'idle' && !error && (
        <div className="empty-state" style={{ flex: 1, justifyContent: 'center', gap: 32 }}>
          ...
          <p className="heading-sm" style={{ marginBottom: 0 }}>{t('waitingPlaceholder')}</p>
          ...
          <p className="platforms-caption">yt-dlp поддерживает <strong>1000+</strong> сервисов</p>
        </div>
      )}
    </div>
  )
