...
function parseMetadataResponse(stdout: string, url: string): VideoMetadata {
  ...
  let chapters: ChapterInfo[] | undefined = parsedData.chapters?.map(
    (c, i): ChapterInfo => ({
      id: String(i),
      title: c.title,
  ...
  // Filter out fake chapters generated by yt-dlp when there is only an accidental timestamp in description
  if (chapters && chapters.length > 0) {
    const hasUntitled = chapters.some((c) => /<untitled chapter/i.test(c.title))
    if (chapters.length < 2 || (hasUntitled && chapters.length <= 2)) {
      chapters = undefined
    }
  }
